...
async function loginWebsite() {
  ...
  try {
    ...
    const stillOnLogin =
      (await page.locator("#Username").count().catch(() => 0)) > 0 &&
      (await page.locator("#Password").count().catch(() => 0)) > 0;
...
async function searchOpenStampConfirm(page, cardSerial, fullName) {
  ...
  if (openClicked) {
    ...
    await page.selectOption("#slStampCode", "260");
    ...
    const selectedValue = await page
      ...
      .inputValue()
      .catch(() => "");
    stampSelected = selectedValue === "260";
  ...
  return {
    ...
    confirmClicked,
    popupConfirmClicked,
    selectedStampValue: stampSelected ? "260" : null,
...
async function main() {
  try {
    ...
    for (const row of rows) {
      const fullName = clean(row["Full Name (Thai, without title/prefix)"]);
      const cardSerial = clean(row["Parking Card ID (Ex.CV000160)"]);
      const vehicleReg = clean(
        row[
          "Vehicle Registration Number (Please provide complete and accurate information)"
        ],
      );
